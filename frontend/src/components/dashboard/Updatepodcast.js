...
export default function Updatepodcast(
 ...
 //upload cover
  ...
  //upload content
  const uploadMedia =()=>{
    axios.post("/podcasts/content-upload" , formDataContent).then(res=>{
  ...
  const handleUpdate = async () => {
    ...
    axios
      .patch(`/podcasts/update/${id}/${selectedpodcast}`, data)
      .then((res) => {
        setSuccMsg("podcast Updated successfully!");
      })
      .catch((err) => {
        setErrMsg("Podcast not found")
      });
  };
  useEffect(() => {
    setErrMsg("");
        ...
        <>
          <div className="absolute inset-0 z-10 overflow-y-auto ">
            <div
              ...
              onClick={() => getupdatepodcast(false)}
            ></div>
